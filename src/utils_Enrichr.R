...
enrichGeneList.oddsRatio <- function(gene.list, databases = "KEGG_2016", fdr.cutoff = 0.1, genome.size=20000) {
  ...
  if(any(res[[1]] == '"expired": true')) stop("Error with Enrichr request, Enrichr seems to not be working currently")
  ...
  if(nrow(enrichRes) > 0){
    ...
    for(enrClass in unique(enrichRes$database)){
      ...
      if(nrow(enrichResX) > 0){
        p <- enrichr.plot(enrichResX, ...)
        if(nrow(p$data) > 0){
          ggsave(paste0(out, label, "_", enrClass, ".pdf"), plot=p,
               width=min(29, 6+ length(unique(enrichResX$grp))*0.3), height=min(29, length(unique(enrichResX$category))*0.3 + 4))
        }
      }
    }
  }
...
fisher.test.enrichment <- function(geneSets, gene.list, bg=NULL, cores=1){
  require(doMC)
  registerDoMC(cores=cores)
  if(is.null(bg)) bg <- do.call(c, gene.list)
  bg <- unique(bg)
  ret <- foreach(gl = names(gene.list)) %dopar% {
    res <- data.table()
